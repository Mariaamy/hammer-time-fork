...
export const AuthProvider = ({children}) => {
    ...
    const [user, setUserState] = useState(() => localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user')) : null);
    ...
    const setUser = (newUser) => {
        ...
        if(!decoded)
            ...
            localStorage.setItem('user', JSON.stringify(decoded)); // Only sets decoded object, NOT ACTUAL access token in localStorage.
    }
    ...
    const register = async (email, password) => {
        try {
            const response = await hAPI.Axios.post(`/auth/register`, {email, password});
